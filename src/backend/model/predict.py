...
def load_model():
    ...
    model.load_state_dict(torch.load("src/backend/model/saved_model/model_card_classifier.pth", map_location=device))
    model.eval()
...
def load_index_to_label_map():
    with open('src/backend/model/saved_model/target_to_class.json', 'r') as file:
